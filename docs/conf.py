...
project = pycomsa.__name__
author = re.match('(.*) <.*>', pycomsa.__author__).group(1)
year = datetime.date.today().year
copyright = '{}, {}'.format("2025" if year==2025 else "2025-{}".format(year), author)

# extract the semantic version
semver = semantic_version.Version.coerce(pycomsa.__version__)
version = str(semver.truncate(level="patch"))
release = str(semver)

# patch the docstring of `pycomsa` so that we don't show the link to redirect
# to the docs (we don't want to see it when reading the docs already, duh!)
doc_lines = pycomsa.__doc__.splitlines()
...
html_theme_options = {
    ...
    "icon_links": [
        ...
        {
            "name": "PyPI",
            "url": "https://pypi.org/project/pycomsa",
    ...
    "switcher": {
        "json_url": "https://pycomsa.readthedocs.io/en/latest/_static/switcher.json",
...
htmlhelp_basename = pycomsa.__name__
